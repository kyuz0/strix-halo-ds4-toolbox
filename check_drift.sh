...
echo "------------------------------------------------------------------"
printf "%-8s | %-12s | %-12s | %-8s | %-8s | %-10s\n" "Tokens" "Max Drift" "RMS Drift" "CPU Top" "GPU Top" "Argmax Match"
echo "------------------------------------------------------------------"
...
for tokens in "${TOKEN_COUNTS[@]}"; do
    ...
    # Parse the output
    # Example output: ds4: Metal prompt graph logits: tokens=18 logits_max=4.23758 logits_rms=0.734869 cpu_top=2581 gpu_top=2581 cpu_top_logit=35.828 gpu_top_logit=36.7067
    ...
    printf "%-8s | %-12s | %-12s | %-8s | %-8s | %-10s\n" "$actual_tokens" "$logits_max" "$logits_rms" "$cpu_top" "$gpu_top" "$match"
